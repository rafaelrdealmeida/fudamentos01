...
#TODO: inserir as infos em um arquivo JSON - ok
...
def extrair_infos(percorre_paginas):
    ...
    for link in percorre_paginas:
        ...
        for nota_imprensa in notas_imprensa: # [nota01,nota02...]
            ...
            print(paragrafos)
            inserir_infos_banco(titulo, link, data, horario, numero_nota,data_atualizada,horario_atualizado,paragrafos)
            print("###")
...
def inserir_infos_banco(titulo, link, data, horario, numero_nota,data_atualizada,horario_atualizado,paragrafos):
    """
    reponsavel por crir o arquivo json. 
    Armazena os dados coletados
    """
    bd = TinyDB("nota_mre.json", indent=4, ensure_ascii=False)
    buscar = Query()
    verificar_bd = bd.contains(buscar.link == link) # True ou False
    if not verificar_bd:
        print("Não esta na base. Inserindo agora...")
        bd.insert({
            "titulo": titulo,
            "link": link,
            "data": data,
        ...
        print("Já esta na base. Não será inserido")
